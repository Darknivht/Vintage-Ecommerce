...
class VintageModern {
    ...
    updateComparisonCount(count) {
        const comparisonBadge = document.getElementById('comparisonCount');
        if (comparisonBadge) {
            comparisonBadge.textContent = count || 0;
            comparisonBadge.classList.add('scale-in');
            setTimeout(() => {
                comparisonBadge.classList.remove('scale-in');
            }, 300);
        }
    }
    ...
    initComparison() {
        document.addEventListener('click', (e) => {
            if (e.target.matches('[data-action="compare"]') || e.target.closest('[data-action="compare"]')) {
                e.preventDefault();
                const btn = e.target.matches('[data-action="compare"]') ? e.target : e.target.closest('[data-action="compare"]');
                const productId = btn.dataset.productId;
                if (productId) {
    ...
    toggleComparison(productId) {
        if (!this.checkAuthentication()) {
            this.showNotification('Please login to use comparison feature', 'warning');
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                this.showNotification(data.message || 'Error updating comparison', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            this.showNotification('Error updating comparison', 'error');
        });
    }
    ...
    showQuickView(productId) {
        ...
        // Load product data
        fetch(`${window.location.origin}/ajax/product-quick-view/${productId}/`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    showQuickView(productId) {
        ...
        .then(data => {
            if (data.success) {
                this.displayQuickViewModal(data.html, data.product_data);
            } else {
    ...
    displayQuickViewModal(htmlContent, productData) {
        const modal = document.createElement('div');
        ...
        this.initializeModalInteractions(modal, productData);
    }

    initializeModalInteractions(modal, productData) {
        // Add event listeners for buttons in the modal
        const addToCartBtn = modal.querySelector('[data-action="add-to-cart"]');
        const wishlistBtn = modal.querySelector('[data-action="wishlist"]');
        const compareBtn = modal.querySelector('[data-action="compare"]');
        ...
        if (wishlistBtn) {
            wishlistBtn.addEventListener('click', () => {
                this.toggleWishlist(productData.id, wishlistBtn);
            });
        }
        
        if (compareBtn) {
            compareBtn.addEventListener('click', () => {
                this.toggleComparison(productData.id);
            });
        }
    }
